...
import { getAllUserAccommodation } from "../services/userAccommodationService";

export default function FilterComponent({ user, setTempSearchParams, tempSearchParams, price, setPrice, handleSortClick, handleResetFilters }) {
    const [locations, setLocations] = useState([]);

    useEffect(() => {
        const fetchLocations = async () => {
            const data = await getAllUserAccommodation(user.token, { limit: 1000 });
    ...
    return (
        <div id="filter" className="w-full md:w-1/4 lg:w-1/5 p-3 border border-grey-100 rounded-lg md:mr-5 mb-5 md:mb-0">
            <h2 className="mb-2 text-lg font-semibold text-gray-900 dark:text-black border-b border-grey-100">Szűrési feltételek beállítása</h2>
            ...
            <div className="border-b-2 mt-2 pb-2">
                ...
                <ul className="max-w-full space-y-1 text-gray-500 list-none list-inside dark:text-gray-800">
            ...
            <div className="mt-2">
                ...
                <ul className="max-w-full space-y-1 text-gray-500 list-none list-inside dark:text-gray-800">
                    {['Reggeli', 'Parkoló', 'Wifi', 'Erkély', 'Étterem', 'Szobaszerviz', 'Állat vihető', 'Lift', 'Franciaágy', 'Wellness', 'Edzőterem'].map((service) => (
                        <li key={service}>
                            <label className="flex items-center">
                                <input
                                    type="checkbox"
